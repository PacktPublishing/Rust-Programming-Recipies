...
fn login(dt:Form<LoginData>,db:DPool,ss:State<session::Session>,mut cookies:Cookies)->Result<impl Responder<'static>,DoodleWebErr>{
    ...
    let user = vals.iter().next().ok_or(DoodleWebErr::UserDoesNotExistErr)?;
...
pub fn ask_question(dt:Form<QuestionForm>,st:State<session::Session>,cookies:Cookies,db:DPool)->Result<impl Responder<'static>,DoodleWebErr>{
